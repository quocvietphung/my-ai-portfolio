...
export default function Home() {
    ...
    const handleSection = async (key: string) => {
        if (key.startsWith("__chat:")) {
            ...
            // Luôn reset chatHistory chỉ còn prompt hiện tại
            ...
            // Chỉ giữ lại câu trả lời này, không nối vào lịch sử cũ
            setChatHistory([
            ...
            setSection(key);
            setChatHistory([]); // Khi chuyển section cũng xóa luôn lịch sử chat
        }
    };
    ...
    const userBg = "#fff";
    const aiBg = "#e0f2fe";
    const userBorder = "#60a5fa";
    const aiBorder = "#0ea5e9";

    // Layout vars
    const AVATAR_HEADER_HEIGHT = 110;
    const AVATAR_HEADER_MARGIN = 15;
    const CONTROL_SEGMENT_HEIGHT = 92;
    const CHATBOX_BOTTOM_MARGIN = 15;

    return (
        <>
            <ParticlesBackground />

            {/* AvatarHeader nổi top */}
            <Box
                pos="fixed"
                top="32px"
                left="50%"
            ...
            >
                ...
                <Flex
                    ...
                    align="center"
                    justify="center"
                    pt={`${AVATAR_HEADER_HEIGHT + AVATAR_HEADER_MARGIN + 32}px`}
                    pb={`${CONTROL_SEGMENT_HEIGHT + CHATBOX_BOTTOM_MARGIN}px`}
                >
                    <Box
                        ref={chatRef}
                        w={["98vw", "90vw", "700px"]}
                    ...
                    >
                        <VStack gap={4} align="stretch" w="100%">
                            {chatHistory.length === 0 ? (
                                <Text color="#aaa" align="center" pt={7}>Ask me anything!</Text>
                            ) : (
                                chatHistory.map((m, i) => (
                                    <Flex
                                        key={i}
